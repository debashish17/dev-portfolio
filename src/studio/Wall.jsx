...
export default function Wall({ onOpenPoster, onRemix, onMakeYours, focusId }) {
  ...
  const load = useCallback(async (p = period) => {
    setState('loading');
    try {
      const d = await studioApi.wall(p);
      setData(d);
    ...
    <div className="st-wall">
        ...
        <div className="st-wall-stats">
          {data?.configured && (
            <div className="mono st-dim" style={{ textAlign: 'right' }}>
              <div><span className="serif-display st-red" style={{ fontSize: 22 }}>{data.counts?.likes ?? 0}</span> LIKES · {data.counts?.posters ?? 0} POSTERS THIS WEEK</div>
      ...
      {state === 'ok' && (
        <>
          {top.length === 0 ? (
            <div className="st-card st-wall-msg">
              <div className="display" style={{ fontSize: 20 }}>{period === 'mine' ? 'NOTHING OF YOURS YET.' : 'THE WALL IS EMPTY.'}</div>
              <p>{period === 'mine' ? 'Posters you publish from this browser show up here.' : 'The first poster published this week takes spot one. Make yours.'}</p>
            </div>
          ) : (
            <ol className="st-ten">
              {top.map((p, i) => (
                <li key={p.id} className={`st-ten-item ${i === 0 && period !== 'latest' && period !== 'mine' ? 'is-first' : ''} ${p.id === focusId ? 'is-focus' : ''}`}>
                  <button type="button" className="st-card-poster clickable" onClick={() => onOpenPoster(p.id)} data-magnet>
                    ...
                    {period !== 'latest' && period !== 'mine' && <span className={`st-rank serif-display ${i === 0 ? 'is-red' : ''}`}>{i + 1}</span>}
